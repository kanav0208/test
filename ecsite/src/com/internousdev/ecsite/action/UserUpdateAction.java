...
public class UserUpdateAction extends ActionSupport implements SessionAware{
	...
	public String execute(){
		...
		if(!(loginUserId.length() == 0) && !(userAddress2.length() == 0) && !
		(userName.length() == 0)
				&& !(userAddress1.length() == 0l) && !(userAddress2.length() == 0)
				&& !(userAddress3.length() == 0) && !(tell.length() == 0)){

			if(loginUserId.equals(checkLoginId) &&
			loginPassword.equals(checkPassword)){

				String defaultLoginId = ((LoginDTO)
				session.get("loginUser")).getLoginId();
				String defaultPassword =((LoginDTO)
				session.get("loginUser")).getLoginPassword();
				session.put("defaultLoginId", defaultLoginId);

				if( defaultLoginId.equals(loginUserId) ||
				defaultPassword.equals(loginPassword)){
					...
					if(defaultLoginId.equals(loginUserId)){
						...
						//IDを変更する場合
						dateCheck =
						userCreateConfirmDAO.getUserInfo(loginUserId, loginPassword);
					}
				}else{
					//パスワード、ID 両方変更する場合
				...
				if(dateCheck){
					...
					setErrorMassage("同じID又はパスワードが存在します。");
					result = ERROR;
				}
			}else{
